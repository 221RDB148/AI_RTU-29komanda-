...
from Linards_minmax import minimax
from Linards_AlphaBeta import alphabeta
...
def main() -> None:
    ...
    def has_errors() -> bool:
        ...
        try:
            ...
            if not 50 <= n <= 70:  # ! Var nomainīt uz 0, lai testētu ar mazāk akmeņiem
    ...
    def ai_move(game_tree, turn, all_moves=False) -> None:
        ...
        if algorithm == "Min-max algoritms":
            move = minimax(game_tree, game_state.turn.get(), game_state.node_id, 70, True)[1]
            game_state.node_id = move.node_id
            rocks_taken = game_state.totalrocks.get() - move.rocks
        elif algorithm == "Alfa-beta algoritms":
            move = alphabeta(game_tree, game_state.turn.get(), game_state.node_id, 70, True)[1]
            game_state.node_id = move.node_id
            rocks_taken = game_state.totalrocks.get() - move.rocks
        confirm_move(rocks_taken)

        # Rekursija, lai automātiski veiktu visus gājienus
