...
@lru_cache(10000)
def alphabeta(game_tree, level, node_id, depth, is_maximizing, alpha=-math.inf, beta=math.inf):
    ...
    if is_maximizing:
        ...
        for node_id2 in game_tree[level][node_id].children:
            score_eval = alphabeta(game_tree, level + 1, node_id2, depth - 1, False)[0]
        ...
        min_eval = math.inf
        for node_id2 in game_tree[level][node_id].children:
            score_eval = alphabeta(game_tree, level + 1, node_id2, depth - 1, True)[0]
...
def main():
    ...
    move = alphabeta(game_tree_start, 0, 0, max_depth, True)[1]
    ...
    while 1:
        ...
        move = alphabeta(game_tree_start, levels, move.node_id, max_depth, True)[1]
        if levels % 2 == 0:
            print("P1 takes", rock_buffer - move.rocks, " and moves =")
        else:
